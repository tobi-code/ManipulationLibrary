...
cpdef double[:,::1] region_filter_cython(double[:] center, double[:,::1] pcd, int hand_cloud, double[:] hand_center, double[:] obj_center, double median_distance):
    ...
    if hand_cloud:
        ...
        for i in range(length):
            dist_temp = distance_cython(pcd[i], hand_center)
            if dist_temp < dist:
                dist = dist_temp
                first_point = pcd[i]
    else:
        #find the nearest point in the point cloud from middle point
            for i in range(length):
                dist_temp = distance_cython(pcd[i], obj_center)
                if dist_temp < dist:
                    dist = dist_temp
                    first_point = pcd[i]
